...
\thispagestyle{myheadings}
	...
	\section{Range and point queries in a persistent model}\label{section:range-queries-persistent}
		...
		\subsection{Experimental evaluation}
			...
			Client \user{} and server \server{} are located in geographically distributed regions, with the ping time between the regions \SI{12}{\milli\second} and effective bandwidth \SI{150}{\mega\byte\per\second}.
			In this proposal I will focus on comparing \epsolute{} to relevant systems and on \epsolute{}'s scalability.
			I refer to the full work for the rest of experimental findings \cite[Section 6]{epsolute}.

			\subsubsection{Against relevant solutions}

				We compared \epsolute{} to conventional \acrshort{sql} \glsdisp{rdbms}{RDBMS's}, PostgreSQL and MySQL, to a linear scan approach and to Shrinkwrap \cite{shrinkwrap}.
				\glsdisp{rdbms}{RDBMS's} were configured for maximum performance and zero security.
				Linear scan is a strawman approach, which downloads the entire server state every query, decrypts the dataset and then runs the query locally.
				Shrinkwrap \cite{shrinkwrap} is a construction that executes federated \acrshort{sql} queries securely.
				...
				\epsolute{} is efficient --- while answering a query in \SI{840}{\milli\second}, it is only 8 to 4 times slower than a typical \acrlong{rdbms}, 18 times faster than the scan, and three orders of magnitude faster than the Shrinkwrap, see \cref{figure:mechanisms}.

			\subsubsection{Scalability}

				To measure scalability we have run parallel \epsolute{} with shared and separate \acrshort{dp}-sanitized datasets \serverDS{} for varying number of threads \oramsNumber{}.
