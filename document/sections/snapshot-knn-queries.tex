...
.
	...
	\section{Security against attacks}\label{section:knn-snapshot:attacks}
		...
		\subsection{Experimental evaluation}\label{section:knn-snapshot:attacks:experiments}
			...
			\subsubsection{Public model}
				...
				We also see that the metric values drop further as $\beta$ increase, starting slowly for small $\beta$, then accelerating for $\beta$ 10 to 30 and than slowing down again.
				Interestingly, we observe that the attack efficiency dives below the lower baseline, the random embeddings.
				We conclude that $\beta \approx 27$, which is about 2.5 times the maximum length of the input vector, produces the security equivalent to symmetric encryption.
	...
	\section{Search accuracy against security tradeoff}
		...
		We note that the accuracy degrades faster than attack efficiency, which implies that there is a tradeoff between functionality and security.
		Depending on the accuracy loss that the application can tolerate, the plot on \cref{figure:knn-tradeoff} can tell what the level of protection against the inversion attack would be at that level.
		This plot also demonstrates the levels of accuracy and security for the $\beta$ as a derivative of the dataset, in particular, the length of the longest embedding vector $\max N$.

		We observe that at $\beta = \sqrt{\max N}$, the accuracy corresponds to a plaintext dataset and the attack efficiency has dropped significantly compared to the plaintext version.
		We also see that at $\beta = \max N$ both accuracy and attack efficiency drop insubstantially compared to $\beta = \sqrt{\max N}$.
		...
		\emph{Given the significant drop of attack efficiency for smaller $\beta$ while retaining almost optimal search accuracy, we conclude that the method of applying a \acrlong{dcpe} to embedding to secure \acrshort{knn} queries is efficient.}
		The system we developed is also highly tuneable, with the $\beta$ corresponding to the application-specific accuracy and security requirements.
	...
	\section{Conclusions}
		...
		\subsubsection{Future Work}
			...
			An immediate deeper analysis of inversion attack over encrypted embeddings may include an evaluation of the words that the attack model returns beyond simple matching against a set of stop-words.
			...
			Another direction can be running other attack attacks against the embedding, beyond the inversion attack.
			These may be some of the \textcite{embedding-attacks} or adapting lots of general attacks against \acrshort{ml} models, like membership attacks \cite{membership-attacks,enhanced-membership-attacks}.
