...
\thispagestyle{myheadings}
	...
	\section{\texorpdfstring{\acrlong{dcpe}}{Distance Comparison Preserving Encryption}}
		...
		Using a common \acrshort{ope} scheme over vector values to encrypt to object for the purpose of running \acrshort{knn} queries on them has been explored in \cite{quick-n}, but this approach incurs high overhead linear in dimensionality.
		...
		\subsection{\texorpdfstring{\acrshort{dcpe}}{DCPE} construction}
			...
			The key participates in generating the random coins needed to produce deterministic execution, and the amplification factor controls the magnitude of projection of a the plaintext object into the ciphertext.
			...
			That is, the point in the $\beta$-hypersphere is deterministically set from the nonce (``\textbf{n}umber used only \textbf{once}'', unique per point) and the key (one for all points), and the final ciphertext is scaled the same way for all points.
	...
	\section{\texorpdfstring{\acrshort{knn}}{kNN} search accuracy}\label{section:knn-snapshot:search}
		...
		\subsection{Experimental evaluation}
			...
			\subsection{Results for varying $\beta$}
				...
				For first the set of $\beta$ values, we ranged the parameter from 0 to 50 with increments of $1.0$, see \cref{figure:knn-search-coarse}.
				...
				\emph{We therefore conclude that the bit-security \acrshort{dcpe} offers comes a low search accuracy penalty.}
	...
	\section{Conclusions}
		...
		In this work, we developed and analyzed a mechanism for answering \acrshort{knn} queries securely in an outsourced setting.
		...
		We analyzed the correlation between the accuracy and security, and concluded the approach provides meaningful and tunable security and attack resiliency guarantees for a comparatively small penalty in search accuracy.
