\section{Experimental Evaluation}\label{section:range-persistent:experiments}
	...
	For symmetric encryption in \acrshort{oram} we have used \acrshort{aes} in CBC mode \cite{aes-nist,nist-modes} with a 256-bits key (i.e., $\eta_2 = 2^{-256}$), for the hash algorithm \algo{H} used to partition records among \acrshortpl{oram} we have used \acrshort{sha}-256 algorithm \cite{nist-hash}.
