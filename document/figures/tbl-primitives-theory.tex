...
\begin{sidewaystable}
	...
	}\label{table:primitive-usage-theory}
	\begin{tabular*}{\linewidth}{ !{\extracolsep\fill} l c c c c c } % chktex 26

		\toprule

		\multirow{2}{*}{Scheme}							& \multicolumn{2}{c}{Primitive usage (number of invocations)}																				& \multirow{2}{*}{\makecell{Ciphertext size, \\ or state size (bits)}}					&  \multirow{2}{*}{\makecell{Leakage \\ (on top of total order)}}							\\ \cline{2-3}
		...
		\cite{clww-ore}									& $n$ (32) \acrshort{prf} 													& none															& $2n$ (64)																				& \textbf{\makecell{Most-significant \\ differing bit}}										\\

		\midrule

		\multirow{3}{*}{\cite{lewi-wu-ore}}				& \boldmath{} $\nicefrac{2n}{d}$ \unboldmath{} \textbf{(32) \acrshort{prp}}	& \multirow{3}{*}{$\frac{n}{2d}$ (9) Hash}						& 																						& \multirow{3}{*}{\makecell{Most-significant \\ differing block}}							\\
